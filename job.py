...
class Job(object):
    def __init__(self, name, applications, submission_time,
                 ...
                 cache_speedups=False, h_unaware=False, category=None, use_gpr=False):
        ## Job attributes
        self.name = name
        ...
        # flag to control use of Gaussian Process Regression for performance modeling
    ...
    def optimize_perf_params(self, perf_profile):
        ...
        perf_params = fit_perf_params(
                num_nodes, num_replicas, local_bsz, compute_time, step_time, use_gpr=self.use_gpr)
        return perf_params
    ...
    def get_goodput_fn(self, cluster_name=None):
        ...
        if grad_params is None or perf_params is None:
            ...
            return GoodputFunction(perf_params, grad_params, app.init_batch_size, use_gpr=self.use_gpr)
    ...
    def update_local_bsz(self, placement):
        ...
        print("Max local bsz:", max_local_bsz)
    ...
    def step(self, seconds, interference=0.0):
        ...
        while seconds > 0 and self.completion_time is None:
            ...
            print("Estimated Step Time:", batch_size / speedup._goodput_fn.throughput(num_nodes, num_replicas, self.atomic_bsz, self.accum_steps))
            print("Estimated Goodput:", speedup._goodput_fn.efficiency(batch_size) * 
                  speedup._goodput_fn.throughput(num_nodes, num_replicas, self.atomic_bsz, self.accum_steps) / speedup._goodput_fn._init_batch_size * self.calibration_factor)
            print("estimated efficiency: ", speedup._goodput_fn.efficiency(batch_size))

            if self.epoch == 0 or self.epoch == 50 or self.epoch == 80:    
                print("Plotting for epoch")
                speedup = self.get_speedup_fn("aws")
                speedup._goodput_fn.VERBOSE=True
                speedup._goodput_fn.name=self.name
                speedup._goodput_fn.epoch=self.epoch

                atomic_bsz_t = self.atomic_bsz
                accum_steps_t = self.accum_steps
                # Changed
                assert accum_steps_t == 0 

                print("Params: ", self.perf_params["aws"])
                replicas = 1
                while replicas <=64:
                    nodes = int(math.ceil(replicas/4.0))
                    batchsize = replicas * atomic_bsz_t * (accum_steps_t + 1)
                    steptime_est = batchsize / speedup._goodput_fn.throughput(nodes, replicas, atomic_bsz_t, accum_steps_t)
                    
                    placement_t = list()
                    replica_t = replicas
                    while replica_t > 0:
                        if replica_t >= 4:
                            placement_t.append(4)
                            replica_t -= 4
                        else:
                            placement_t.append(replica_t)
                            replica_t = 0   
                    true_steptime, _ = application.get_throughput(tuple(placement_t), atomic_bsz_t)
                    if true_steptime != -1:
                    # Efficiency = speedup._goodput_fn.efficiency(batchsize)
                        print("estimated step time and true step time", self.name, self.epoch, replicas, steptime_est, true_steptime)
                    replicas += 1
                    speedup._goodput_fn.VERBOSE=True
