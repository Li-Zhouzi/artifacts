...
python3 multi_simulator.py --policy dummy --b ${1} --disable_bsz_tuning --oracle_num_nodes=16 --oracle_num_replicas=64 workloads/profile_cifar10.csv --interval 1 | grep -e "^speedup " | sed 's/speedup //' | sed 's/ /,/g' >> ${2}/${1}gpu.csv