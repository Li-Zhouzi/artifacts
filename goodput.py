...
class GoodputFunction(object):
    def __init__(self, perf_params, grad_params, init_batch_size, use_gpr=False):
        if use_gpr:
            # For GPR model, perf_params contains the trained model and scaler
            self._gpr_model = perf_params['model']
            self._scaler = perf_params['scaler']
            self._use_gpr = True
        else:
    ...
    def evaluate(self, num_nodes, num_replicas, atomic_bsz, accum_steps):
        ...
        return self.throughput(num_nodes, num_replicas, atomic_bsz,
                               accum_steps) * self.efficiency(batch_size) / self._init_batch_size

    def throughput(self, num_nodes, num_replicas, atomic_bsz, accum_steps):
        if self._use_gpr:
            # if np.any(accum_steps != 0):
            #     raise ValueError("GPR model does not support accum_steps > 0")
            # Use GPR model to predict step time
            step_time = self._predict_step_time_gpr(num_nodes, num_replicas, atomic_bsz) * (accum_steps + 1)
    ...
    def _predict_step_time_gpr(self, num_nodes, num_replicas, atomic_bsz):
        # Check if inputs are scalars or arrays
        if np.isscalar(num_nodes) and np.isscalar(num_replicas) and np.isscalar(atomic_bsz):
            # Handle scalar case
            features = np.array([[atomic_bsz, num_nodes, num_replicas]])
            features_scaled = self._scaler.transform(features)
            prediction = self._gpr_model.predict(features_scaled)
            return prediction[0]
        else:
            # Handle array case - ensure all arrays have same shape
            # Convert to arrays if they're not already
            atomic_bsz = np.asarray(atomic_bsz)
            num_nodes = np.asarray(num_nodes)
            num_replicas = np.asarray(num_replicas)
            
            # Broadcast to same shape if needed
            shape = np.broadcast(atomic_bsz, num_nodes, num_replicas).shape
            atomic_bsz = np.broadcast_to(atomic_bsz, shape)
            num_nodes = np.broadcast_to(num_nodes, shape)
            num_replicas = np.broadcast_to(num_replicas, shape)
            ...
            features = np.column_stack([
                atomic_bsz.flatten(),
                num_nodes.flatten(),
                num_replicas.flatten()
            ])
            
            # Get predictions
            features_scaled = self._scaler.transform(features)
            predictions = self._gpr_model.predict(features_scaled)
...
def fit_perf_params(num_nodes, num_replicas, atomic_bsz,
                    accum_step_time, optim_step_time, use_gpr=False):
    if use_gpr:
        return fit_perf_params_gpr(num_nodes, num_replicas, atomic_bsz,
                                  accum_step_time, optim_step_time)
    else:
        # Original analytical model fitting
...
def fit_perf_params_gpr(num_nodes, num_replicas, atomic_bsz,
                       accum_step_time, optim_step_time):
    """Fit a Gaussian Process Regression model to predict step time."""
    from sklearn.gaussian_process import GaussianProcessRegressor
    from sklearn.gaussian_process.kernels import RBF, WhiteKernel, ConstantKernel
    from sklearn.preprocessing import StandardScaler
    
    # Prepare features and target
    X = np.column_stack([atomic_bsz, num_nodes, num_replicas])
    y = optim_step_time  # Using optim_step_time as the target
    ...
    # Return the trained model and scaler as a dictionary
